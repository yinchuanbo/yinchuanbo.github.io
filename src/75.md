...
> js 遵循[执行才编译](https://developer.mozilla.org/docs/Web/JavaScript/EventLoop#run-to-completion)的原理，也就是说，只有一个任务结束才会执行下一个任务，而且**不论这个任务会阻塞主线程多久**。
...
> 当*让步*于主线程后，浏览器就有机会处理那些更重要的任务，而不是放在队列中排队。理想状态下，一旦出现用户界面级别的任务，就应该让步给主线程，让任务更快的执行完。**让步于主线程让更重要的工作能更快的完成**
