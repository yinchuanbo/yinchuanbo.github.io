...
目前，`IdleDetector` 在 `Chrome 94+` 、`Edge 94+` 中都支持，`Firefox` 和 `Safari` 尚未支持。但是 `IdleDetector` 仍为实验性 `API`，未来可能有变化，且在非 `HTTPS` 环境下无法使用。
...
async function startIdleDetection(threshold = 60000) {
  ...
  try {
    ...
    const handleStateChange = () => {
      ...
      console.log(
        `[${now}] 用户状态: ${detector.userState}, 屏幕状态: ${detector.screenState}`
      );
      // 实际业务逻辑可以在这里添加
