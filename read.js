...
const articlesDir = path.join(__dirname, "articles/read");
...
removeHtml(path.join(__dirname, "read"));
...
function handleEditTemp(filename = "", html = "", title = "", tag = "") {
  ...
  const htmlName = `./read/${filename.replace("md", "html")}`;
...
function renderHome() {
  ...
  const homeOutputFilePath = path.join(__dirname, `./read/index.html`);
  const keys = Object.keys(tagsNav);
  let homeHTML = `<ul>`;
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    const first = tagsNav[key][0];
    homeHTML += `<li class="card"><a href="/read/${first.replace(
      ".md",
      ".html"
